    ...
    protected function tableColumns(): array
    {
        return [
            ...
            ProgressBarColumn::make('memoryUsage')
                ...
                ->warningThresholdPercent(static::WARNING_THRESHOLD)
                ->dangerThresholdPercent(static::DANGER_THRESHOLD)
                ->maxValue(fn (Server $server) => ServerResourceType::MemoryLimit->getResourceAmount($server) === 0 ? ($server->node?->statistics()['memory_total'] ?? 0) : ServerResourceType::MemoryLimit->getResourceAmount($server))
            ...
            ProgressBarColumn::make('diskUsage')
                ...
                ->warningThresholdPercent(static::WARNING_THRESHOLD)
                ->dangerThresholdPercent(static::DANGER_THRESHOLD)
                ->maxValue(fn (Server $server) => ServerResourceType::DiskLimit->getResourceAmount($server) === 0 ? ($server->node?->statistics()['disk_total'] ?? 0) : ServerResourceType::DiskLimit->getResourceAmount($server))
