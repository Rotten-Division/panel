...
test('variant maps each ServerState to the expected pill style', function () {
    expect((new StatePill(ServerState::Nest))->variant())->toBe('nest');
    expect((new StatePill(ServerState::Hydrating))->variant())->toBe('nest');
    expect((new StatePill(ServerState::Capturing))->variant())->toBe('nest');
...
test('transferring flag overrides any state', function () {
    $pill = new StatePill(ServerState::Nest, ContainerStatus::Running, transferring: true);
...
test('label upper-cases the state value and replaces underscores', function () {
    ...
    expect((new StatePill(ServerState::Nest))->label())->toBe('NEST');
});
...
test('pulses returns true only for transient and installing variants', function () {
    ...
    expect((new StatePill(ServerState::Nest))->pulses())->toBeFalse();
