...
class NodeHealthServiceTest extends IntegrationTestCase
{
    ...
    {
        $node = Node::factory()->create(['last_seen_at' => Carbon::now()->subSeconds(60)]);
    ...
    {
        $node = Node::factory()->create(['last_seen_at' => Carbon::now()->subSeconds(180)]);
    ...
    {
        $fresh = Node::factory()->create(['last_seen_at' => Carbon::now()->subSeconds(10)]);
        $stale = Node::factory()->create(['last_seen_at' => Carbon::now()->subSeconds(300)]);
        $never = Node::factory()->create(['last_seen_at' => null]);
    ...
    {
        Node::factory()->create(['last_seen_at' => null]);
        $this->assertFalse($this->service->hasHealthyNode());

        Node::factory()->create(['last_seen_at' => Carbon::now()]);
